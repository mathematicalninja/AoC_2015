...
class Functions(TestCase):
    ...
    def test_getCoordValue(self):
        for pair in [[0, 0], [1, 1]]:
            with self.subTest(pair=pair):
                self.assertEqual(
                    getCoordValue(coord=pair, lightbulbs=self.lightbulbs), 0
                )

        for pair in [[1, 0], [0, 1]]:
            with self.subTest(pair=pair):
                self.assertEqual(
                    getCoordValue(coord=pair, lightbulbs=self.lightbulbs), 1
                )
    ...
    def test_actOnCoord_off(self):
        ...
        for coord in [self.o_o, self.o_z, self.z_o, self.z_z, self.t_t]:
            with self.subTest(coord=coord):
                self.assertEqual(
                    getCoordValue(
                        coord, actOnCoord(coord, "off", self.lightbulbs, part=1)
                    ),
                    0,
                )
    ...
    def test_actOnCoord_on(self):
        ...
        for coord in [self.o_o, self.o_z, self.z_o, self.z_z, self.t_t]:
            with self.subTest(coord=coord):
                ...
                self.assertEqual(
                    getCoordValue(
                        coord, actOnCoord(coord, "on", self.lightbulbs, part=1)
                    ),
                    1,
                )
    ...
    def test_actOnCoord_toggle(self):
        ...
        self.assertEqual(
            getCoordValue(
                self.o_o, actOnCoord(self.o_o, "toggle", self.lightbulbs, part=1)
            ),
            1,
        )
        self.assertEqual(
            getCoordValue(
                self.o_z, actOnCoord(self.o_z, "toggle", self.lightbulbs, part=1)
            ),
            0,
        )
        self.assertEqual(
            getCoordValue(
                self.z_o, actOnCoord(self.z_o, "toggle", self.lightbulbs, part=1)
            ),
            0,
        )
        self.assertEqual(
            getCoordValue(
                self.z_z, actOnCoord(self.z_z, "toggle", self.lightbulbs, part=1)
            ),
            1,
        )
        self.assertEqual(
            getCoordValue(
                self.t_t, actOnCoord(self.t_t, "toggle", self.lightbulbs, part=1)
            ),
            1,
        )
